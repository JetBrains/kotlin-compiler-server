...
fun parseComposeRequest(event: JsonNode, mapper: ObjectMapper, log: LambdaLogger): ComposeWasmRequest? {
    ...
    return when {
        path.endsWith("/translate/compose-wasm") -> tryParse<ComposeWasmV2Request>(mapper, body, "V2", log)
        path.endsWith("/translate") && compiler == "compose-wasm" -> tryParse<ProjectRequest>(mapper, body, "V1", log)
        else -> null
    }
}
