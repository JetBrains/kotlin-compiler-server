...
dependencies {
    ...
    kotlinDependency("org.jetbrains.kotlinx:kotlinx-coroutines-core-jvm:1.5.2")
...
fun generateProperties(prefix: String = "", outputDirectoryPath: String = "") = """
    ...
    ${if (outputDirectoryPath.isNotEmpty()) "outputDirectoryPath=${outputDirectoryPath}" else ""}
""".trimIndent()
...
val buildLambda by tasks.creating(Zip::class) {
    ...
    val lambdaOutputDirectoryPath = "/tmp/"
    from(tasks.compileKotlin)
    from(tasks.processResources) {
        eachFile {
            if (name == propertyFile) { file.writeText(generateProperties(lambdaWorkDirectoryPath, lambdaOutputDirectoryPath)) }
        }
    }
    from(policy)
    from(indexes)
